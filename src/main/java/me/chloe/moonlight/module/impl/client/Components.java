...
import me.chloe.moonlight.module.impl.combat.Killaura;
import me.chloe.moonlight.module.impl.render.Nametags;
...
public class Components extends Module {
    ...
    private void doChestCounter() {
        ...
        Moonlight.fontUtil.drawStringWithShadow("Chests: " + getChestCount(), setX, setY, Colors.INSTANCE.getColor(setY, false));
        if(doubleChestCounter.getValue()) {
            Moonlight.fontUtil.drawStringWithShadow("Dubs: " + Math.floor(getChestCount() / 2), setX, setY+10, Colors.INSTANCE.getColor(setY+10, false));
        }
    }
    ...
    private void doPvPInfo() {
        ...
        if(xp.getValue()) {
            Moonlight.fontUtil.drawStringWithShadow("XP " +String.valueOf(getItemCount(Items.EXPERIENCE_BOTTLE)), setX, setY, Colors.INSTANCE.getColor(setY, false));
            setY+=10;
        }
        if(crystals.getValue()) {
            Moonlight.fontUtil.drawStringWithShadow("Crystals " + String.valueOf(getItemCount(Items.END_CRYSTAL)), setX, setY, Colors.INSTANCE.getColor(setY, false));
            setY+=10;
        }
        if(obi.getValue()) {
            Moonlight.fontUtil.drawStringWithShadow("Obi " + String.valueOf(getItemCount(Item.getItemFromBlock(Blocks.OBSIDIAN))), setX, setY, Colors.INSTANCE.getColor(setY, false));
            setY+=10;
        }
        if(totems.getValue()) {
            Moonlight.fontUtil.drawStringWithShadow("Totems " +String.valueOf(getItemCount(Items.TOTEM_OF_UNDYING)), setX, setY, Colors.INSTANCE.getColor(setY, false));
            setY+=10;
        }
        if(gapples.getValue()) {
            Moonlight.fontUtil.drawStringWithShadow("Gapples " +String.valueOf(getItemCount(Items.GOLDEN_APPLE)), setX, setY, Colors.INSTANCE.getColor(setY, false));
        }
    }
    ...
    private void doTargetHud() {
        ...
        Moonlight.fontUtil.drawStringWithShadow(entName, setX, setY, 0xffffffff);
        setY+= Moonlight.fontUtil.getFontHeight()+2+5;
        int armorY = 0;
        ...
        if(targetHealth.getValue()) {
            ...
            Moonlight.fontUtil.drawStringWithShadow("HP: "+ textColor + String.valueOf(pHealth), setX+20, setY, 0xffffffff);
            setY+=10;
        }

        if(targetPopped.getValue()) {
            if(Nametags.getTotemPops(targetEntity.getName()) > 0) {
                Moonlight.fontUtil.drawStringWithShadow("TP: "+ TextFormatting.RED + String.valueOf(Nametags.getTotemPops(targetEntity.getName())), setX+20, setY, 0xffffffff);
                setY+=10;
            }
        }
        if(targetPing.getValue()) {
            Moonlight.fontUtil.drawStringWithShadow("Ping: "+ TextFormatting.GREEN + EntityUtil.getPing((EntityPlayer) targetEntity), setX+20, setY, 0xffffffff);
            setY+=10;
        }
        if(targetBackground.getValue()) {
            double maxLength = setX + 63.5 + (EntityUtil.getPing((EntityPlayer) targetEntity) > 100 ? Moonlight.fontUtil.getStringWidth("9") : 0);
    ...
    private int renderArmor(int x, int setY, EntityPlayer entity) {
        ...
        for (final ItemStack is : stacks) {
            ...
            Moonlight.fontUtil.drawStringWithShadow(dmg + "", (int)((x + 8 - Moonlight.fontUtil.getStringWidth(dmg + "") / 2)+16), y+3, RenderUtil.toRGBA((int)(red * 255.0f), (int)(green * 255.0f), 0));
            y+=16;
        }
        GlStateManager.enableDepth();
        GlStateManager.disableLighting();
    ...
    private Entity getClosestTarget() {
        ...
        if(t == null) {
            if(targetHudPrio.getValue() == me.chloe.moonlight.module.impl.client.Components.THudPrio.CAURA && Killaura.INSTANCE != null) {
                t = mc.world.getPlayerEntityByName(Killaura.INSTANCE.getModuleInfo());
            } else if(targetHudPrio.getValue() == me.chloe.moonlight.module.impl.client.Components.THudPrio.KAURA) {
                t = getClosest();
        ...
        if(t == null) {
            if(targetHudPrio.getValue() == me.chloe.moonlight.module.impl.client.Components.THudPrio.CAURA) {
                t = getClosest();
            } else if(targetHudPrio.getValue() == me.chloe.moonlight.module.impl.client.Components.THudPrio.KAURA && AutoCrystal.INSTANCE != null) {
                t = mc.world.getPlayerEntityByName(AutoCrystal.INSTANCE.getModuleInfo());
            ...
            if (e != null) {
                if(e != mc.player) {
                    if (e instanceof EntityPlayer) {
                        if(!Moonlight.friendManager.isFriend(e.getName())) {
