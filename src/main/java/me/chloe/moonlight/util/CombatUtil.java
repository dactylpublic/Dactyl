...
import me.chloe.moonlight.injection.inj.access.IBlock;
import me.chloe.moonlight.injection.inj.access.IVec3i;
...
public class CombatUtil {
    ...
    public static boolean crystalIsBreakable(boolean multiPoint, EntityEnderCrystal crystal, AutoCrystal.WallsRange rangeMode, double breakTraceRange, boolean antiSuicide, double maxSelfDmg, AutoCrystal.PassLogic passLogic, double doesDMGMin, double enemyRange) {
        ...
        if(passLogic == AutoCrystal.PassLogic.DOESDMG) {
            ...
            List<Entity> entities = new ArrayList<Entity>((Collection<? extends Entity>) mc.world.playerEntities.stream()
                    ...
                    .filter(entityPlayer -> !Moonlight.friendManager.isFriend(entityPlayer.getName()))
    ...
    public static boolean placePosStillSatisfies(BlockPos blockPos, boolean antiSuicide, double maxSelfDmg, double minDamage, double startFacePlaceHealth, boolean doPlaceTrace, double placeTraceRange, double enemyRange, boolean onepointthirteen, double placeRange) {
        ...
        final List<Entity> playerEntities = new ArrayList<Entity>((Collection<? extends Entity>) mc.world.playerEntities.stream().filter(entityPlayer -> !Moonlight.friendManager.isFriend(entityPlayer.getName())).collect(Collectors.toList()));
    ...
    public static BlockPos getBestPlacePosition(boolean antiSuicide, double maxSelfDmg, double minDamage, double startFacePlaceHealth, boolean doPlaceTrace, double placeTraceRange, double enemyRange, boolean oneBlockCA, double placeRange) {
        ...
        final List<Entity> playerEnts = new ArrayList<Entity>((Collection<? extends Entity>) mc.world.playerEntities.stream().filter(entityPlayer -> !Moonlight.friendManager.isFriend(entityPlayer.getName())).collect(Collectors.toList()));
        double damage = 2.0;
