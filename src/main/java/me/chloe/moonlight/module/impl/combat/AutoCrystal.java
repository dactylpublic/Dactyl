...
import me.chloe.moonlight.injection.inj.access.ICPacketUseEntity;
...
public class AutoCrystal extends Module {
    ...
    public Setting<Boolean> customLookAtPoses = new Setting<Boolean>("Rotation Offsets", false, v->isViewGeneral());
    public Setting<Double> customLookAtX = new Setting<Double>("X", 0.5d, 0.0d, 1.0d, v->isViewGeneral() && customLookAtPoses.getValue());
    public Setting<Double> customLookAtY = new Setting<Double>("Y", 0.5d, 0.0d, 1.0d, v->isViewGeneral() && customLookAtPoses.getValue());
    ...
    private void breakCrystals() {
        ...
        EntityEnderCrystal crystal = (EntityEnderCrystal) mc.world.loadedEntityList.stream()
                ...
                .filter(entity -> CombatUtil.passesStrictBreak((EntityEnderCrystal) entity, true, true, ticksExisted.getValue()))
    ...
    private void placeCrystals() {
        ...
        if (placePosition != null) {
            ...
            if(noPlaceAttack.getValue()) {
                if(currentAttacking == null) {
                    EntityEnderCrystal crystal = (EntityEnderCrystal) mc.world.loadedEntityList.stream()
                            ...
                            .filter(entity -> CombatUtil.wontSelfPop((EntityEnderCrystal) entity, antiSuiBreak.getValue(), maxSelfBreak.getValue()))
                            .filter(entity -> CombatUtil.passesStrictBreak((EntityEnderCrystal) entity, true, false, ticksExisted.getValue()))
                            .min(Comparator.comparing(entity -> mc.player.getDistance(entity)))
                            .orElse(null);
            ...
            if(customLookAtPoses.getValue()) {
                double x = customLookAtX.getValue();
                double y = customLookAtNegY.getValue() ? -customLookAtY.getValue() : customLookAtY.getValue();
                double z = customLookAtZ.getValue();
                finalRots = CombatUtil.calculateLookAt(placePosition.getX() + x, placePosition.getY() + y, placePosition.getZ() + z);
            }
    ...
    public void onRender3D(Render3DEvent event) {
        ...
        if (damageText.getValue()) {
            if (placeRender != null && CombatUtil.isHoldingCrystal()) {
                List<Entity> playerEnts = new ArrayList<Entity>((Collection<? extends Entity>) mc.world.playerEntities.stream().filter(entityPlayer -> !Moonlight.friendManager.isFriend(entityPlayer.getName())).collect(Collectors.toList()));
                        ...
                        if (packetSpawnObject.getType() == 51) {
                            ...
                            if (placedOnPos(pos.down())) {
                                for (EntityPlayer p : mc.world.playerEntities) {
                                    if (p == null || mc.player.equals(p) || p.getDistanceSq(pos) > (((enemyRange.getValue() + placeRange.getValue())) * ((enemyRange.getValue() + placeRange.getValue()))) || !Moonlight.friendManager.isFriend(p.getName())) {
                                        continue;
                                    }
