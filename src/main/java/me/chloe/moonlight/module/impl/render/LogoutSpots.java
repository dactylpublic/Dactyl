...
public class LogoutSpots extends Module {
    ...
    private void renderNameTag(final String n, final double x, final double yi, final double z, final float delta, final double xPos, final double yPos, final double zPos) {
        ...
        int width = (int) (Moonlight.fontUtil.getStringWidth(getDisplayName(name, xPos, yPos, zPos)) / 2);
        ...
        if(!Moonlight.fontUtil.isCustomFont()) {
            Nametags.drawBorderedRect((-width - 2), -(Moonlight.fontUtil.getFontHeight() + 1), width + 2.0F, 1F, 1.0F, 1996488704, clr);
        } else {
            Nametags.drawBorderedRect((-width - 2), -(Moonlight.fontUtil.getFontHeight() + 2), width + 2.0F, 1F, 1.0F, 1996488704, clr);
        }
        ...
        if(!Moonlight.fontUtil.isCustomFont()) {
            Moonlight.fontUtil.drawStringWithShadow(getDisplayName(name, xPos, yPos, zPos), -width, (int) -(Moonlight.fontUtil.getFontHeight() - 1), clr);
        } else {
            Moonlight.fontUtil.drawStringWithShadow(getDisplayName(name, xPos, yPos, zPos), -width, (int) -(Moonlight.fontUtil.getFontHeight() + 2), clr);
        }
