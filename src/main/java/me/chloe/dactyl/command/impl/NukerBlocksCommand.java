...
public class NukerBlocksCommand extends Command {
    ...
    private void handleIncorrectUsage() {
        ...
        ChatUtil.printMsg("Usage: "+ Dactyl.commandManager.getPrefix() +"nukerblocks <add/del> <blockname>", true, false);
    }
    ...
    private void handleAddBlock(String[] args) {
        ...
        if(!Dactyl.nukerManager.hasBlock(Block.getBlockFromName(args[2]))) {
            Dactyl.nukerManager.addBlock(args[2]);
    ...
    private void handleRemoveBlock(String[] args) {
        if(Dactyl.nukerManager.hasBlock(Block.getBlockFromName(args[2]))) {
            Dactyl.nukerManager.removeBlock(args[2]);
